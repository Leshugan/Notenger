...
export default function App() {
  ...
  async function clipWrite(t){
    clipText.current=t;
    let ok=false;
    try{ const m=await import("@capacitor/clipboard"); await m.Clipboard.write({string:t}); ok=true; }catch{}
  ...
  async function clipRead(){
    try{ const m=await import("@capacitor/clipboard"); const r=await m.Clipboard.read(); if(r&&typeof r.value==="string") return r.value; }catch{}
  ...
  function openHdrMenu(type,e) { e.stopPropagation(); setHdrMenu(v=>v===type?v:type); }
            ...
            return (
              <div key={f.id} data-fid={f.id} className="row" onClick={()=>openF(f)}
                ...
                style={{display:"flex",alignItems:"center",gap:14,padding:"12px 14px",position:"relative",
                  cursor:"pointer",transition:"transform .18s ease, box-shadow .18s ease, background .1s",borderBottom:"1px solid #241C16",
        ...
        {composerFull && (
          <div
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 10px",borderTop:"1px solid #3A2E24",background:"#241C16",flexShrink:0,minHeight:46}}>
              <button onClick={()=>setFullFmt(v=>!v)} title="Форматирование"
              ...
              <div style={{position:"absolute",left:10,bottom:64,background:"#241C16",borderRadius:12,padding:"5px 6px",
                ...
                {[
                  ...
                  <button key={i} onMouseDown={e=>e.preventDefault()}
                    onClick={()=>{ const el=fullTaRef.current; if(!el)return; const s=el.selectionStart,e2=el.selectionEnd,sel=note.slice(s,e2)||it.x; setNote(note.slice(0,s)+it.b+sel+it.a+note.slice(e2)); setTimeout(()=>{el.focus();const p=s+it.b.length+sel.length+it.a.length;el.setSelectionRange(p,p);},0); }}
