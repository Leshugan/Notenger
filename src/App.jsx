        ...
        <div ref={scrollRef} onScroll={updateActiveNote}
          ...
          {snotes.map(n=>{
            ...
            <div key={n.id}
              ...
              <div onClick={(e)=>{ if(e.target!==e.currentTarget) return; if(selectMode||multiSelect.length>0) return; const now=Date.now(); if(rowTap.current.id===n.id && now-rowTap.current.t<300){ if(scrollRef.current) preserveScroll.current=scrollRef.current.scrollTop; justEnteredSel.current=n.id; setTimeout(()=>{justEnteredSel.current=null;},450); setSelectMode(n.id); rowTap.current={id:null,t:0}; } else { rowTap.current={id:n.id,t:now}; } }}
